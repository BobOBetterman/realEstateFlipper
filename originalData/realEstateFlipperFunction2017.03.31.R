...
addLatLong <- function(propListingsUpdate) {
  ...
  streetList <- ldply(seq(1, nrow(propListingsUpdate)), function(x) 
    {tryCatch(street2coordinates(propListingsUpdate$propertyAddresses[x]), error=function(err) as.data.frame(NA))})
...
addCompsReport <- function(longListReport) {
  ...
  propSoldOneYear <- propListSoldNew[propListSoldNew[,18] >= oneYear, ]
  # propSoldOneYear <- propSoldOneYear[propSoldOneYear$Postal.City != "",]
  # 
  # propAddressList <- paste(propSoldOneYear$Street.Address, propSoldOneYear$Postal.City, "CA", 
  #                          propSoldOneYear$Zip.Code, sep = ", ")
  # 
  # propSoldOneYear[, "propertyAddresses"] <- propAddressList
