...
emailReports <- function(recipients, subject, clientName, shortTearDownPropReport, shortDiscountPropReport, allFileNames) {
  ...
  if (nrow(shortTearDownPropReport) > 0 & nrow(shortDiscountPropReport) > 0) {
    send.mail(from = sender, to = recipients, subject = subject, body = body,
              ...
              attach.files = c(allFileNames[2], allFileNames[3]))
  } else if (nrow(shortTearDownPropReport) > 0) {
    send.mail(from = sender, to = recipients, subject = subject, body = body,
              smtp = list(host.name = hostName, port = port,
                          user.name = userName, passwd = passWord, ssl = TRUE),
              authenticate = TRUE, send = TRUE,
              attach.files = allFileNames[2])
  } else {
    send.mail(from = sender, to = recipients, subject = subject, body = body,
              smtp = list(host.name = hostName, port = port,
                          user.name = userName, passwd = passWord, ssl = TRUE),
              authenticate = TRUE, send = TRUE,
              attach.files = allFileNames[3])
  }
}
...
emailLongReport <- function(recipients, subject, clientName, longPropReport, allFileNames) {
