...
clientFlipperReport <- function(clientName, buildCost, lowestBreakEvenBuild, cities, recipients) {
  ...
  if(file.exists("flipperStats.csv")) {
    sunnyData <- tearDownUpdate(buildCost, lowestBreakEvenBuild)
  } else {
    sunnyData <- tearDownFlip(buildCost, lowestBreakEvenBuild)
  }
  
  longPropReport <- longProspectsReport(sunnyData, cities)
  
  shortPropReport <- shortProspectsReport(sunnyData, lowestBreakEvenBuild, cities)
  
  writeReports(longPropReport, shortPropReport, clientName)
  
  if (nrow(shortPropReport) > 0) {
    emailReports(recipients, subject, clientName)
  }
}
...
tearDownFlip <- function(newBuildCost, lowestNewBuildCost){
  ...
  propListingsActive[, "DPSResidual"] <- (propListingsActive$list.Price.Num / propListingsActive$houseSqFt) - 
...
# This function does the same as the above (create a data frame of flipping prospects), 
# but this one is called when updating the database.
tearDownUpdate <- function(newBuildCost, lowestNewBuildCost) {
  propListings <- read.csv("flipperAll.csv", stringsAsFactors = FALSE)
  propListingsUpdate <- read.csv("flipperStats.csv", stringsAsFactors = FALSE)
...
shortProspectsReport <- function(activeProp, lowestNewBuildCost, cities) {
  propProspectsReportShort <- subset(activeProp, breakEvenBuildCost >= lowestNewBuildCost & 
                                     ...
                                     select = c("Street.Address", "Postal.City", "lotSqFt", "list.Price.Num",
                                                "houseSizeSqFt", "DPSResidual", "breakEvenBuildCost", 
  ...
  propProspectsReportShort <- propProspectsReportShort[order(-propProspectsReportShort$breakEvenBuildCost, 
                                                             propProspectsReportShort$DPSResidual), ]
...
writeReports <- function(longReport, shortReport, clientName) {
  longFileName <- paste("clientReports/", clientName, "LongProspectsReport.csv", sep = "")
  shortFileName <- paste("clientReports/", clientName, "ProspectsReport.csv", sep = "")
  
  write.csv(longPropReport, longFileName, row.names = FALSE)
  write.csv(shortPropReport, shortFileName, row.names = FALSE)
}



# This function sends the report to interested parties
emailReports <- function(recipients, subject, clientName) {
  ...
  shortFileLocation <- paste("clientReports/", clientName, "ProspectsReport.csv", sep = "")
  ...
  send.mail(from = sender, to = recipients, subject = subject, body = body,
            smtp = list(host.name = hostName, port = port,
                        user.name = userName, passwd = passWord, ssl = TRUE),
            authenticate = TRUE, send = TRUE,
            attach.files = shortFileLocation)
}